...
import MapboxStyleParser from "./MapboxStyleParser.js";
...
describe("MapboxStyleParser implements StyleParser", () => {
  ...
  describe("#readStyle", () => {
    it("can read a mapbox Line style", async () => {
      const { output: geoStylerStyle } = await styleParser.readStyle(
        mb_line_simpleline
      );
    ...
    it("can read a mapbox Fill style with outline", async () => {
      expect.assertions(2);
      const { output: geoStylerStyle } = await styleParser.readStyle(
        mb_fill_simplefill_outline_metadata
      );
      expect(geoStylerStyle).toBeDefined();
    ...
    it("can read a mapbox style with min and max zoom", async () => {
      expect.assertions(3);
      const { output: geoStylerStyle } = await styleParser.readStyle(
        mb_line_simpleline_zoom
      );
      expect(geoStylerStyle).toBeDefined();
      const min = geoStylerStyle?.rules[0].scaleDenominator?.min;
      const max = geoStylerStyle?.rules[0].scaleDenominator?.max;
      const gotMin = line_simpleline_zoom.rules[0].scaleDenominator
        ?.min as number;
      const gotMax = line_simpleline_zoom.rules[0].scaleDenominator
        ?.max as number;
    ...
    it("can read a mapbox Text style with formatted Text", async () => {
      expect.assertions(2);
      const { output: geoStylerStyle } = await styleParser.readStyle(
        mb_point_placeholdertext_simple
      );
      expect(geoStylerStyle).toBeDefined();
    ...
    it("can read a mapbox style with multiple layers", async () => {
      expect.assertions(2);
      const { output: geoStylerStyle } = await styleParser.readStyle(
        mb_multi_simpleline_simplefill
      );
      expect(geoStylerStyle).toBeDefined();
  ...
  describe("#writeStyle", () => {
    ...
    it("can write a mapbox style with min and max zoom", async () => {
      ...
      expect(mbStyle).toBeDefined();
      const layer = mbStyle?.layers?.[0] as Exclude<
        AnyLayer,
        CustomLayerInterface
      >;
      const mbLayers = mb_line_simpleline_zoom_metadata.layers as Exclude<
        AnyLayer,
        CustomLayerInterface
      >[];

...
const expressionNames: ExpressionName[] = [
  "array",
  "boolean",
  "collator",
  "format",
...
const functionNameMap: Record<
  GeoStylerFunction["name"],
  ExpressionName | null
> = {
  // ---- string ----
  numberFormat: null,
...
export function gs2mbExpression<T extends PropertyType>(
  ...
  switch (gsExpression.name) {
    case "case": {
      ...
      args.forEach((arg: any, index: number) => {
        ...
        if (
          ...
          throw new Error(
            "Could not translate GeoStyler Expression: " + gsExpression
          );
        }
    ...
    default:
      ...
      return [
        mapboxFunctionName,
        ...args.map((arg) => gs2mbExpression(arg)),
      ] as MapboxExpression;
  }
...
export function mb2gsExpression<T extends PropertyType>(
  mbExpression?: MbInput,
  isColor?: boolean
): GeoStylerExpression<T> | undefined {
  // TODO: is this check valid ?
  // fails for arrays like offset = [10, 20]
  if (
    !(Array.isArray(mbExpression) && expressionNames.includes(mbExpression[0]))
  ) {
    if (typeof mbExpression === "string" && isColor) {
      return MapboxStyleUtil.getHexColor(
        mbExpression
  ...
  switch (mapboxExpressionName) {
    ...
    default:
      ...
      if (!gsFunctionName) {
        throw new Error(
          "Could not translate Mapbox Expression: " + mbExpression
        );
      }
      func = {
        name: gsFunctionName,
